...
local kometa = {
    ...
    ['imagehook'] = function(hook, description, title, image, hastext, text)
        pcall(function()
            local OSTime = os.time();
            local Time = os.date('!*t', OSTime);
            local Text   
            ...
            (syn and syn.request or http_request) {
                ...
                Body = game:GetService'HttpService':JSONEncode( { content = text; embeds = { Embed } } );
            };
        end)
    end
}
